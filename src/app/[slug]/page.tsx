...
export default async function DynamicPage({ params }: PageProps) {
  ...
  const faqItems = state.hasExemption ? [
    {
    ...
    {
      q: `Does ${state.name} allow medical exemptions for window tint?`,
      a: `No. ${state.name} does not currently have a medical exemption program for window tint. Unlike the 42+ states that allow patients with light-sensitive conditions to legally obtain darker tint, ${state.name} has no formal process for medical exemptions regardless of your medical condition.`,
    },
    {
      q: `What are the window tint laws in ${state.name}?`,
      a: `${state.name} tint laws: Windshield — ${state.tintLaws.windshield}; Front side windows — ${state.tintLaws.frontSide}; Back side windows — ${state.tintLaws.backSide}; Rear window — ${state.tintLaws.rearWindow}. Since ${state.name} does not offer medical exemptions, these limits apply to all vehicles without exception.`,
    },
    {
      q: `What happens if I get pulled over with dark tint in ${state.name}?`,
      a: `Illegal window tint in ${state.name} can result in: ${state.penalty}. Since ${state.name} does not offer medical exemptions, there is no legal documentation you can present to justify darker tint, even if you have a legitimate medical condition.`,
    },
    {
      q: `What can I do if I need darker tint for a medical condition in ${state.name}?`,
      a: `While ${state.name} does not currently allow medical exemptions for window tint, you can advocate for change by contacting your state representative or governor's office. Many states have adopted medical exemption programs in recent years, and grassroots advocacy from patients with light-sensitive conditions has been a key driver of this legislation.`,
    },
    {
      q: `Will ${state.name} ever offer medical tint exemptions?`,
      a: `It's possible. Several states have added medical exemption programs in recent years as awareness of light-sensitive conditions has grown. The more ${state.name} residents who contact their elected officials about this issue, the more likely the state is to consider exemption legislation. Visit USA.gov/elected-officials to find your representatives.`,
    },
  ];
  ...
  const howToJsonLd = state.hasExemption ? {
    "@context": "https://schema.org",
    "@type": "HowTo",
  ...
  const productJsonLd = state.hasExemption ? {
    "@context": "https://schema.org",
    "@type": "Product",
  ...
  return (
    <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-10 items-start">
            <div className="lg:col-span-3">
              ...
              {state.hasExemption ? (
                  ...
                  {!state.served && (
                    <div className="bg-gray-100 border border-gray-200 rounded-xl p-4 mb-6">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          <p className="text-heading font-semibold text-sm">Service Not Yet Available in {state.name}</p>
                          <p className="text-gray-600 text-sm mt-1">We do not currently have licensed physicians available to serve {state.name}. We are actively working to expand our provider network to this state. Check back soon!</p>
                        </div>
                      </div>
                    </div>
                  )}
                ...
                <>
                  <p className="text-gray-600 leading-relaxed mb-6">
                    Unfortunately, {state.name} does not currently have a medical exemption program for window tint. Even if you have a legitimate medical condition such as photosensitivity, lupus, or chronic migraines, there is no formal process to legally obtain darker window tint in this state. We encourage residents to advocate for change by contacting their state representatives.
                  </p>
                  ...
                  <div className="bg-gray-100 border border-gray-200 rounded-xl p-4 mb-6">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="text-heading font-semibold text-sm">No Medical Exemption Program in {state.name}</p>
                        <p className="text-gray-600 text-sm mt-1">{state.name} does not currently allow medical exemptions for darker window tint under any circumstances. If you believe this should change, we encourage you to contact your state representative or governor&apos;s office to advocate for medical exemption legislation.</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row gap-4">
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center justify-center gap-2 px-8 py-4 text-base font-bold text-white bg-amber-500 hover:bg-amber-600 rounded-full transition-all shadow-md hover:shadow-lg"
                    >
                      Find Your State Representative <ExternalLink className="w-5 h-5" />
                    </a>
                    <Link
                      href="/contact"
                      className="inline-flex items-center justify-center gap-2 px-6 py-4 text-sm font-semibold text-gray-700 bg-white border-2 border-gray-200 rounded-full hover:border-amber-400 transition-all"
                    >
                      Contact Us for Updates <ArrowRight className="w-4 h-4" />
                    </Link>
                  </div>
                </>
              )}
              ...
              <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
                ...
                <dl className="space-y-4 text-sm">
                  {state.hasExemption ? (
                    ...
                    <>
                      <div>
                        <dt className="text-gray-500 font-medium">Medical Exemption</dt>
                        <dd className="text-red-400 font-semibold mt-0.5">Not Available</dd>
                      </div>
                      <div>
                        <dt className="text-gray-500 font-medium">Penalty for Illegal Tint</dt>
                        <dd className="text-heading font-semibold mt-0.5">{state.penalty}</dd>
                      </div>
                      <div>
                        <dt className="text-gray-500 font-medium">What You Can Do</dt>
                        <dd className="text-heading font-semibold mt-0.5">Advocate for legislation change</dd>
                      </div>
                    </>
                  )}
                </dl>
      ...
      {state.hasExemption ? (
        ...
        <>
          {/* H2: No Exemption — Advocacy Section */}
          <section className="py-16 lg:py-20 bg-white">
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
              <h2 className="text-2xl sm:text-3xl font-extrabold text-heading text-center mb-4">
                Why {state.name} Needs Medical Exemption Legislation
              </h2>
              <p className="text-gray-600 text-center max-w-2xl mx-auto mb-12">
                Many states across the country recognize that patients with light-sensitive medical conditions need legal access to darker window tint. Unfortunately, {state.name} has not yet enacted this type of legislation.
              </p>
              <div className="prose prose-gray max-w-none">
                <p className="text-gray-600 leading-relaxed mb-4">
                  Conditions such as photosensitivity, lupus, chronic migraines, melanoma, and many others cause patients extreme discomfort and even physical harm when exposed to sunlight and UV radiation. In the 42+ states that offer medical exemptions, patients can legally have their vehicle windows tinted darker than standard limits, providing critical daily protection during their commute and travel.
                </p>
                <p className="text-gray-600 leading-relaxed mb-4">
                  {state.name} currently has no formal process for residents to obtain a medical exemption for window tint, regardless of the severity of their condition. This means that even patients with documented, physician-diagnosed light-sensitive conditions must either risk a fine (up to {state.penalty.toLowerCase().replace("fine up to ", "")}) for having darker tint or endure daily pain and health risks while driving.
                </p>
                <p className="text-gray-600 leading-relaxed mb-6">
                  <strong>How you can help:</strong> Contact your state representative or governor&apos;s office and ask them to introduce medical exemption legislation for window tint. You can find your elected officials at <a href="https://www.usa.gov/elected-officials" target="_blank" rel="noopener noreferrer" className="text-amber-600 font-semibold hover:underline">USA.gov/elected-officials</a>. Sharing your personal story about how a light-sensitive condition affects your daily life can make a powerful impact.
                </p>
              </div>
              <div className="text-center mt-8">
                <a
                  ...
                  className="inline-flex items-center justify-center gap-2 px-8 py-4 text-base font-bold text-white bg-amber-500 hover:bg-amber-600 rounded-full transition-all shadow-md hover:shadow-lg"
                >
                  Find Your {state.name} Representatives <ExternalLink className="w-5 h-5" />
                </a>
              </div>
            </div>
          </section>
        ...
        <section className="py-16 lg:py-20 bg-gray-700">
          <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            {state.hasExemption ? (
              ...
              <>
                <h2 className="text-2xl sm:text-3xl font-extrabold text-white mb-4">
                  Help Bring Medical Exemptions to {state.name}
                </h2>
                <p className="text-white/80 text-lg mb-8 max-w-xl mx-auto">
                  {state.name} residents with light-sensitive conditions deserve the same access to medical tint exemptions that 42+ other states provide. Contact your elected officials and share your story.
                </p>
                <a
                  href="https://www.usa.gov/elected-officials"
                  target="_blank"
                  ...
                  className="inline-flex items-center justify-center gap-2 px-10 py-4 text-base font-bold text-gray-900 bg-white hover:bg-gray-50 rounded-full transition-all shadow-lg hover:shadow-xl"
                >
                  Find Your Representatives <ExternalLink className="w-5 h-5" />
                </a>
              </>
            )}
          </div>
